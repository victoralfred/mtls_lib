...
void platform_secure_zero(void *ptr, size_t len)
{
    ...
    /* Use explicit_bzero if available (glibc), otherwise volatile loop */
    /* Note: explicit_bzero is glibc-specific, not available on macOS */
    /* On macOS, use volatile loop to prevent optimization */
    volatile unsigned char *ptr_bytes = (volatile unsigned char *)ptr;
    while (len--) {
        *ptr_bytes++ = 0;
    }
}
