...
mtls_socket_t platform_socket_create(int domain, int type, int protocol, mtls_err *err)
{
    ...
    if (sock == MTLS_INVALID_SOCKET) {
        MTLS_ERR_SET(err, MTLS_ERR_SOCKET_CREATE_FAILED, "Failed to create socket: %d",
                     WSAGetLastError());
        if (err) {
...
{
    (void)nonblocking; /* Parameter used in mode assignment */
    u_long mode = nonblocking ? 1 : 0;
    if (ioctlsocket(sock, FIONBIO, &mode) != 0) {
        MTLS_ERR_SET(err, MTLS_ERR_INTERNAL, "Failed to set non-blocking mode: %d",
                     WSAGetLastError());
        if (err) {
            err->os_errno = WSAGetLastError();
        }
...
int platform_socket_set_recv_timeout(mtls_socket_t sock, uint32_t timeout_ms, mtls_err *err)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (const char *)&timeout, sizeof(timeout)) != 0) {
        MTLS_ERR_SET(err, MTLS_ERR_INTERNAL, "Failed to set recv timeout: %d", WSAGetLastError());
        if (err) {
            err->os_errno = WSAGetLastError();
        }
        return -1;
...
int platform_socket_set_send_timeout(mtls_socket_t sock, uint32_t timeout_ms, mtls_err *err)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (const char *)&timeout, sizeof(timeout)) != 0) {
        MTLS_ERR_SET(err, MTLS_ERR_INTERNAL, "Failed to set send timeout: %d", WSAGetLastError());
        if (err) {
            err->os_errno = WSAGetLastError();
        }
        return -1;
...
int platform_socket_set_reuseaddr(mtls_socket_t sock, bool enable, mtls_err *err)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt, sizeof(opt)) != 0) {
        MTLS_ERR_SET(err, MTLS_ERR_INTERNAL, "Failed to set SO_REUSEADDR: %d", WSAGetLastError());
        if (err) {
            err->os_errno = WSAGetLastError();
        }
        return -1;
...
int platform_socket_bind(mtls_socket_t sock, const mtls_addr *addr, mtls_err *err)
{
    if (bind(sock, &addr->addr.sa, addr->len) != 0) {
        ...
        MTLS_ERR_SET(err, platform_socket_error_to_mtls(wsa_err), "Failed to bind socket: %d",
                     wsa_err);
        if (err) {
...
int platform_socket_listen(mtls_socket_t sock, int backlog, mtls_err *err)
{
    if (listen(sock, backlog) != 0) {
        MTLS_ERR_SET(err, MTLS_ERR_SOCKET_LISTEN_FAILED, "Failed to listen on socket: %d",
                     WSAGetLastError());
        if (err) {
            err->os_errno = WSAGetLastError();
        }
...
mtls_socket_t platform_socket_accept(mtls_socket_t sock, mtls_addr *addr, mtls_err *err)
{
    ...
    if (client == MTLS_INVALID_SOCKET) {
        MTLS_ERR_SET(err, MTLS_ERR_ACCEPT_FAILED, "Failed to accept connection: %d",
                     WSAGetLastError());
        if (err) {
            err->os_errno = WSAGetLastError();
        }
                            ...
                            mtls_err *err)
{
    ...
    if (timeout_ms > 0) {
        ...
        if (ret == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK) {
            int wsa_err = WSAGetLastError();
            MTLS_ERR_SET(err, platform_socket_error_to_mtls(wsa_err), "Connect failed: %d",
                         wsa_err);
            if (err) {
                err->os_errno = wsa_err;
            }
        ...
        if (ret == SOCKET_ERROR) {
            ...
            if (ret == SOCKET_ERROR) {
                MTLS_ERR_SET(err, MTLS_ERR_CONNECT_FAILED, "Select failed: %d", WSAGetLastError());
                if (err) {
                    err->os_errno = WSAGetLastError();
                }
                return -1;
            ...
            if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (char *)&sock_err, &len) != 0) {
                MTLS_ERR_SET(err, MTLS_ERR_INTERNAL, "getsockopt failed: %d", WSAGetLastError());
                if (err) {
                    err->os_errno = WSAGetLastError();
                }
                return -1;
            ...
            if (sock_err != 0) {
                MTLS_ERR_SET(err, platform_socket_error_to_mtls(sock_err), "Connection failed: %d",
                             sock_err);
                if (err) {
...
int platform_parse_addr(const char *addr_str, mtls_addr *addr, mtls_err *err)
{
    ...
    if (ret != 0) {
        MTLS_ERR_SET(err, MTLS_ERR_DNS_FAILED, "DNS resolution failed: %d", ret);
        return -1;
    }
