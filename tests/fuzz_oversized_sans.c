...
static size_t prng_mod_size(size_t mod)
{
    if (mod == 0) {
        return 0;
    }
    return (size_t)(prng_next_u32() % (uint32_t)mod);
}
...
static char *generate_random_string(size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        unsigned char char_value =
            (unsigned char)(0x20U + (unsigned char)prng_mod_size((0x7EU - 0x20U + 1U)));
...
static char *generate_dns_string(size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        str[i] = DNS_CHARS[prng_mod_size(charset_len)];
    }
    str[len] = '\0';
...
static bool test_boundary_exact_limit(void)
{
    ...
    (void)mtls_memcpy_s(str2, MTLS_MAX_IDENTITY_LEN + 1, str1, MTLS_MAX_IDENTITY_LEN + 1);
...
static bool test_fuzz_random_lengths(void)
{
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        if (i % 10 == 0) {
            /* 10% over limit */
            len = MTLS_MAX_IDENTITY_LEN + 1 + prng_mod_size(5000);
            should_accept = false;
        } else {
            /* 90% within limit */
            len = 1 + prng_mod_size(MTLS_MAX_IDENTITY_LEN);
            should_accept = true;
        }
...
static bool test_fuzz_dns_patterns(void)
{
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        size_t len = 10 + prng_mod_size(MTLS_MAX_IDENTITY_LEN + 100);
...
static bool test_san_validation_both_oversized(void)
{
    ...
    (void)mtls_memcpy_s(oversized_allowed, MTLS_MAX_IDENTITY_LEN + 51, oversized_san,
                        MTLS_MAX_IDENTITY_LEN + 51);
...
static char *duplicate_string(const char *str)
{
    ...
    if (dup) {
        (void)mtls_memcpy_s(dup, len + 1, str, len + 1);
    }
    return dup;
}
...
int main(void)
{
    /* Initialize random seed (not a fixed constant) */
    g_rng_state = (uint32_t)time(NULL) ^ (uint32_t)FUZZ_SEED;
