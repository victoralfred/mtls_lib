...
static void test_hostname_buffer_overflow(void)
{
    ...
    if (ctx) {
        ...
        (void)mtls_memset_s(long_hostname, sizeof(long_hostname), 'a', 255);
        long_hostname[255] = '\0';
        char addr[300];
        ...
        assert(err.code == MTLS_ERR_INVALID_ADDRESS || err.code == MTLS_ERR_CONNECT_FAILED ||
               err.code == MTLS_ERR_CA_CERT_NOT_FOUND);

        /* Test with hostname one char over limit (256 chars) */
        char too_long[257];
        (void)mtls_memset_s(too_long, sizeof(too_long), 'a', 256);
        too_long[256] = '\0';
